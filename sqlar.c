...
///
/// 200508 use getpass if stdin is a tty, else read password from stdin
///

static char les_passbuf[MX_PASSPHRASE+1];

static char *les_getpass(const char *zPrompt) {
	
	
	// use getpass if stdin is a tty 
	if (isatty(STDIN_FILENO)) {
		return getpass(zPrompt);
	}
	// stdin is not a tty. read password from it
	fgets(les_passbuf, MX_PASSPHRASE, stdin);
	return les_passbuf;
}
...
static void prompt_for_passphrase(
  ...
  int doScramble,         /* Scramble the input if true */ ///ignore it
  char *zPassphrase       /* Write result here */
){
  char *z;
  int i;
  z = les_getpass(zPrompt);
  while( isspace(z[0]) ) z++;
